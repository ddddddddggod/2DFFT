...
//              Signed-safe version for FFT data path
//----------------------------------------------------------------------
...
module FFT2D_Buffer #(
    ...
    input               wr_en,
    input  signed [WIDTH-1:0]  wr_re,
    input  signed [WIDTH-1:0]  wr_im,

    // Read Interface (Output to Doppler FFT)
    output reg          rd_valid,
    output signed [WIDTH-1:0]  rd_re,
    output signed [WIDTH-1:0]  rd_im,
    ...
    localparam N = 128;
    localparam TOTAL_SAMPLES = N * N; // 16384
    ...
    always @(posedge clock or posedge reset) begin
        if (reset) begin
            ...
            if (rd_active) begin
                rd_valid <= 1'b1;
                if (rd_addr_linear == TOTAL_SAMPLES - 1)
                    rd_addr_linear <= 0;
                else
    ...
    //------------------------------------------------------------------
    // 2. Memory Core (BRAM, Signed Data)
    //------------------------------------------------------------------
    (* ram_style = "block" *) reg signed [WIDTH-1:0] mem_re_0 [0:TOTAL_SAMPLES-1];
    (* ram_style = "block" *) reg signed [WIDTH-1:0] mem_im_0 [0:TOTAL_SAMPLES-1];
    (* ram_style = "block" *) reg signed [WIDTH-1:0] mem_re_1 [0:TOTAL_SAMPLES-1];
    (* ram_style = "block" *) reg signed [WIDTH-1:0] mem_im_1 [0:TOTAL_SAMPLES-1];

    reg signed [WIDTH-1:0] dout_re_0, dout_im_0;
    reg signed [WIDTH-1:0] dout_re_1, dout_im_1;
